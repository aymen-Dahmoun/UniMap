...
def get_shortest_path(
    start_type: str = Query(..., description="Type of the start node: 'room', 'landmark', or 'node'"),
    start_id: int = Query(..., description="ID of the start room or navigation node"),
    end_type: str = Query(..., description="Type of the end node: 'room', 'landmark', or 'node'"),
    end_id: int = Query(..., description="ID of the end room or navigation node"),
    db: Session = Depends(get_db)
):
    """
    Compute the shortest path between two nodes (rooms, landmarks, or nav nodes)
    """
    path_finder = PathFinder()
    path_finder.build_graph_from_db(db)

    # Fetch all Nodes for debug if needed
    # nodes = db.query(Nodes).all()
    # print("All Nodes:")
    # for n in nodes:
    #     print(f"Node ID: {n.id}, Kind: {n.node_kind}, Floor: {n.floor}")

    # Fetch all Paths for debug if needed
    # paths = db.query(Paths).all()
    # print("\nAll Paths:")
    # for p in paths:
    #     print(f"Path ID: {p.id}, {p.start_node_id} -> {p.end_node_id}, Distance: {p.distance}")
    ...
    # Find shortest path using Nodes
